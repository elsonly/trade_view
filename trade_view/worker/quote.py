...
class QuoteWorker:
    ...
    def __init__(
        ...
        if len(subscribe_codes) * len(quote_types) > self.SUBSCRIBE_LIMIT:
            raise Exception(
                f"number of subscription to subscribe beyond limit: {self.SUBSCRIBE_LIMIT}"
            )

        self.save_quote_types = quote_types.copy()
        if "001" in subscribe_codes:
            if "tick" in quote_types:
                self.save_quote_types.append("market_tick")
            if "orderbook" in quote_types:
                self.save_quote_types.append("market_info")
